...
MunitResult test_bitmove_all(const MunitParameter params[], void * data) {
	...
	// using the extraction functions.
	{
		...
		munit_assert_string_equal("e2e4", e2e4str);
		free(e2e4str);
	}
...

	{
		...
		const BitMove ba1h8 = BitMove__new(a1, h8, PROMO_NONE);
		...
		munit_assert_string_equal("a1h8", ba1h8str);
		free(ba1h8str);
	}

	{
		...
		const BitMove d4c3ep = BitMove__new(d4, c3, PROMO_NONE);
		...
		munit_assert_string_equal("d4c3", d4c3epstr);
		free(d4c3epstr);
	}

	{
		...
		const BitMove oo = BitMove__new(e1, g1, PROMO_NONE);
		...
		munit_assert_string_equal("e1g1", oostr);
		free(oostr);
	}

	{
		...
		const BitMove oo = BitMove__new(e1, c1, PROMO_NONE);
		munit_assert_int(e1, ==, BitMove__from(oo));
		munit_assert_int(c1, ==, BitMove__to(oo));
		munit_assert_int(PROMO_NONE, ==, BitMove__promoted_piece(oo));
		munit_log(MUNIT_LOG_INFO, BitMove__to_notation(oo));
		char * oostr = BitMove__to_notation(oo);
		munit_assert_string_equal("e1c1", oostr);
		free(oostr);
	}
